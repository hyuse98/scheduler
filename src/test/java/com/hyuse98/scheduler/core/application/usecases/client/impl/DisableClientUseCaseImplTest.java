...
class DisableClientUseCaseImplTest {

    @Mock
    private ClientRepository clientRepository;

    @InjectMocks
    private DisableClientUseCaseImpl disableClientUseCase;
    ...
    void shouldDisableClientSuccessfully() {
        ...
        Client activeClient = buildClient(id, true);

        when(clientRepository.findById(id)).thenReturn(Optional.of(activeClient));
        when(clientRepository.save(any(Client.class))).thenReturn(activeClient);

        disableClientUseCase.execute(id);

        ArgumentCaptor<Client> clientCaptor = ArgumentCaptor.forClass(Client.class);
        verify(clientRepository).save(clientCaptor.capture());

        assertFalse(clientCaptor.getValue().getActive());
    }
